...
import serial
import time
arduino = serial.Serial(port='COM19', baudrate=115200, timeout=0.1)
class Ui_MainWindow(object):
    def keyPressEvent(self,event) :
        ...
        if msg=='y' :
            ...
            self.label_2.setPixmap(QtGui.QPixmap("frwrd-arrow.png"))
            time.sleep(0.05)
        if msg=='a' :
            self.initialiseLabels()
            arduino.write(bytes("56", 'utf-8'))
            self.label.setPixmap(QtGui.QPixmap("bckwrd-arrow.png"))
            time.sleep(0.05)
        ...
        if msg=='l' :
            ...
            self.label_9.setPixmap(QtGui.QPixmap("upward-arrow.png"))
            time.sleep(0.05)
            data = arduino.readline()
            print( str(data) )
        if msg=='k' :
            ...
            self.label_8.setPixmap(QtGui.QPixmap("downward-arrow.png"))
            time.sleep(0.05)
            data = arduino.readline()
            print(data)
        if msg=='h' :
            ...
            self.label_3.setPixmap(QtGui.QPixmap("right-arrow.png"))
            time.sleep(0.05)  
        if msg=='p' :
            self.initialiseLabels()
            arduino.write(bytes("6", 'utf-8'))
            self.label_4.setPixmap(QtGui.QPixmap("left-arrow.png"))
            time.sleep(0.05)
        if msg=='s' :
            self.initialiseLabels()
    ...
    def setupUi(self, MainWindow):
        ...
        # camThread2 = threading.Thread(target=self.cameraConn2)
        camThread.start()
        # camThread2.start()
    ...
    def cameraInit2(self) :
        address = ('localhost', 6001)

        listener = Listener(address,authkey=b'secret password')
        self.conn=listener.accept()
        print(f'connection accepted from {listener.last_accepted}')
    def cameraConn2(self) :
        self.cameraInit()
        while True :
            msg = self.conn.recv()
            self.label_21.setPixmap(self.convertTopix(msg))
